let headerItems = document.querySelectorAll(
  ".burger-content ul.menu .menu-item"
);
...
const getImageName = (e) => {
  if (e.innerText.split("\n")[0].match(/[a-z]/i)) {
    ...
    return kirToLat(e.innerText.split("\n")[0]);
  }
};
          ...
          try {
            await checkIfImageExists(nameImg).then(() => {
              ...
              if (e.children[2] !== undefined) {
                e.lastChild.style.top = `${
                  e.children[0].offsetHeight + e.children[1].offsetHeight
                }px`;
              } else {
              ...
              e.addEventListener("mouseenter", () => {
                if (
                  e.parentElement.parentElement.classList.contains("hovered")
                ) {
              ...
              if (
                e.lastChild.getBoundingClientRect().x !==
                listOfHeaderItems.getBoundingClientRect().x
              ) {
